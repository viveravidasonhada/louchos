          ...
          <div className="max-w-6xl mx-auto animate-fadeIn space-y-12 pb-20">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
              <div>
                ...
                <p className="text-slate-400 mt-2 font-medium tracking-tight">Estrategista Inteligente • Cloud Sync Ativo</p>
              </div>
              <div className="flex gap-3">
                <button onClick={() => loadInitialData()} className="p-3 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-xl border border-slate-700 transition-all">
            ...
            <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                 <div onClick={() => setView('experts')} className="group bg-slate-800/40 p-8 rounded-2xl border border-slate-700/50 hover:border-amber-500/50 hover:bg-slate-800 transition-all cursor-pointer shadow-lg">
                    ...
                    <p className="text-slate-400 text-sm mb-6 leading-relaxed">Arquétipos e Tom de Voz persistentes.</p>
                    <div className="text-amber-500 text-xs font-black uppercase tracking-widest flex items-center gap-2">
                        {experts.length} Expert(s) <ArrowRight size={14} />
                    </div>
                 </div>
                 <div onClick={() => setView('team')} className="group bg-slate-800/40 p-8 rounded-2xl border border-slate-700/50 hover:border-emerald-500/50 hover:bg-slate-800 transition-all cursor-pointer shadow-lg">
                    ...
                    <p className="text-slate-400 text-sm mb-6 leading-relaxed">Atribua tarefas e mantenha o progresso sincronizado.</p>
                    <div className="text-emerald-500 text-xs font-black uppercase tracking-widest flex items-center gap-2">
                        {team.length} Colaboradores <ArrowRight size={14} />
                    </div>
                 </div>
                 <div onClick={() => setView('campaigns')} className="group bg-slate-800/40 p-8 rounded-2xl border border-slate-700/50 hover:border-indigo-500/50 hover:bg-slate-800 transition-all cursor-pointer shadow-lg">
                    ...
                    <h3 className="text-white font-bold text-xl mb-2">Blueprints</h3>
                    <p className="text-slate-400 text-sm mb-6 leading-relaxed">Modelos de fases base para a IA.</p>
                    <div className="text-indigo-500 text-xs font-black uppercase tracking-widest flex items-center gap-2">
                        {blueprints.length} Modelos <ArrowRight size={14} />
                    </div>
                 </div>
            </section>
            ...
            <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                 {projectsList.map(p => (
                   <div key={p.id} className="relative group">
                     <div onClick={() => p.id && handleSwitchProject(p.id)} className="bg-slate-800/60 p-6 rounded-2xl border border-slate-700 hover:border-indigo-500 transition-all cursor-pointer group hover:shadow-2xl">
                     ...
                     {currentUser === 'admin' && p.id && (
                       <button 
                        ...
                        className="absolute -top-2 -right-2 bg-red-600 p-2 rounded-full text-white shadow-lg opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-500 z-10"
                       >
                         <Trash2 size={14} />
                       </button>
                     )}
  ...
  return (
    <div className="min-h-screen bg-[#0f172a] text-slate-200 selection:bg-indigo-500 selection:text-white flex flex-col font-sans">
      ...
      <footer className="py-10 border-t border-slate-800 text-center">
        <div className="text-[10px] text-slate-600 font-black uppercase tracking-[0.5em] mb-2">LaunchOS Cloud Enterprise v3.1</div>
      </footer>
    </div>
  );
};
