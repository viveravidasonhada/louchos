...
const LaunchDashboard: React.FC<LaunchDashboardProps> = ({ data, currentUser, onUpdateProject, onDeleteProject }) => {
  ...
  const handleUpdateTaskData = (taskId: string, updates: Partial<Task>) => {
    // Bloqueio rigoroso de IDs inválidos
    if (!taskId || taskId === 'undefined') {
        console.warn("Interrompido: Tentativa de atualizar tarefa com ID inválido.");
        return;
    }
    ...
    const newPhases = data.phases.map(phase => ({
      ...
      tasks: phase.tasks.map(t => {
        // A comparação agora é segura porque garantimos IDs no Supabase service
        if (t.id === taskId) {
          return { ...t, ...updates };
        }
        return t;
    ...
    // Se o modal estiver aberto para esta tarefa, atualiza o modal também
    ...
    return (
      <div className="space-y-6 animate-fadeIn">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="bg-slate-800/50 p-6 rounded-xl border border-slate-700 shadow-lg">
            <h3 className="text-slate-400 text-xs font-bold uppercase mb-2 tracking-widest">Entregas Realizadas</h3>
            <div className="text-4xl font-black text-indigo-400">{allTasks.reduce((acc, t) => acc + (t.attachments?.length || 0), 0)}</div>
          </div>
          <div className="bg-slate-800/50 p-6 rounded-xl border border-slate-700 shadow-lg relative group">
            <h3 className="text-slate-400 text-xs font-bold uppercase mb-2 tracking-widest">Ações do Projeto</h3>
        ...
        <div key={phase.id} className="w-80 flex-shrink-0 flex flex-col bg-slate-900/60 rounded-2xl border border-slate-800 shadow-xl max-h-full">
          ...
          <div className="p-3 space-y-3 overflow-y-auto flex-1 custom-scrollbar">
            {(phase.tasks || []).map(task => (
              <div key={task.id} onClick={() => setSelectedTask(task)} className={`p-4 bg-slate-800/80 border border-slate-700 rounded-xl cursor-pointer hover:border-indigo-500/50 transition-all ${task.status === 'done' ? 'grayscale opacity-50' : ''}`}>
                <div className="flex justify-between items-start mb-2">
                  <span className="text-[9px] font-black px-2 py-0.5 bg-indigo-500/10 text-indigo-400 rounded-full">{task.assignee}</span>
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-fadeIn">
            {Object.entries(tasksByAssignee).map(([assignee, tasks]) => (
                <div key={assignee} className="bg-slate-900/40 rounded-2xl border border-slate-800 overflow-hidden">
                    <div className="p-4 bg-slate-800/40 border-b border-slate-800 flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            <div className="w-8 h-8 rounded-full bg-indigo-500/20 text-indigo-400 flex items-center justify-center font-bold text-xs">{assignee.substring(0,2).toUpperCase()}</div>
                            <h3 className="text-sm font-bold text-white">{assignee}</h3>
                        </div>
                        <span className="text-[10px] text-slate-500 font-bold uppercase">{tasks.length} Tarefas</span>
                    </div>
                    <div className="p-4 space-y-3">
                        {tasks.map(t => (
                            <div key={t.id} onClick={() => setSelectedTask(t)} className="p-3 bg-slate-800/40 border border-slate-700/50 rounded-xl hover:border-indigo-500 transition-all cursor-pointer">
        ...
        <div className="bg-slate-900/40 rounded-2xl border border-slate-800 p-6 animate-fadeIn">
            ...
            <div className="space-y-4">
                {sortedTasks.map((task) => (
                    <div key={task.id} onClick={() => setSelectedTask(task)} className="group flex items-start gap-4 p-4 hover:bg-slate-800/40 rounded-xl transition-all cursor-pointer">
                        ...
                        <div className="flex-1">
                            ...
                            <p className="text-[10px] text-slate-500 mt-1 uppercase tracking-widest">{task.assignee}</p>
                        </div>
                        <div className="pt-1">
            ...
            <div className="p-8 border-b border-slate-800 flex justify-between items-start bg-slate-900/50">
              <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                      <span className="px-3 py-1 bg-indigo-500/20 text-indigo-400 text-[10px] font-black uppercase rounded-lg">{selectedTask.assignee}</span>
                ...
                <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {(selectedTask.attachments || []).map(att => (
                            <div key={att.id} className="flex items-center justify-between p-4 bg-slate-800/40 border border-slate-800 rounded-2xl group">
